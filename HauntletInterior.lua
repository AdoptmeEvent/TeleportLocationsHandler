...
-- This script is designed to automatically teleport the local player to the HauntletInterior,
-- and then place the character directly onto a specific part (InteriorOrigin) inside the model.
...
-- --- DIRECT TELEPORT TO HAUNTLETINTERIOR ---
local destinationId = "HauntletInterior"
local doorIdForTeleport = "MainDoor" 

-- Function to handle position adjustment and part modification after the teleport is visually complete.
local function handlePostTeleportMovement()
    ...
    local hauntletInterior = nil 
    local interiors = workspace:FindFirstChild("Interiors")
    
    if interiors then
        print("Waiting for HauntletInterior model to appear in Workspace.Interiors...")
        -- Wait up to 10 seconds for the HauntletInterior model to load
        hauntletInterior = interiors:WaitForChild("HauntletInterior", 10) 
    end

    if not hauntletInterior then
        warn("HauntletInterior model did not load within the timeout period.") 
        return
    end
    
    -- Initial wait to ensure the screen fade is completely gone.
    task.wait(1.0) 

    -- NEW TARGET PART PATH: workspace.Interiors.HauntletInterior.InteriorOrigin
    local targetPart = hauntletInterior:FindFirstChild("InteriorOrigin")

    if targetPart and targetPart:IsA("BasePart") then
        
        -- 1. MODIFY PART PROPERTIES (Size and CanCollide)
        print("Modifying InteriorOrigin part properties...")
        targetPart.Size = Vector3.new(10, 1, 10) -- Make it large and a bit thick
        targetPart.CanCollide = true -- Make it solid (as requested)
        
        -- 2. POSITIONING LOGIC
        local safeOffset = 5.0 -- Safer offset to prevent falling through (from previous fix)
        local targetCFrame = targetPart.CFrame + Vector3.new(0, safeOffset, 0)
        local totalTime = 0
        ...
        connection = RunService.Heartbeat:Connect(function(dt)
            ...
            if totalTime > ENFORCEMENT_DURATION then
                ...
                print("Aggressive CFrame enforcement finished. Character should be stable at the InteriorOrigin.")
            end
        end)
        ...
        print("Character teleported directly onto the modified InteriorOrigin part (Aggressive Enforcement Active).")
    else
        warn("Target part (InteriorOrigin) not found inside the loaded HauntletInterior model. Check the path and part name.") 
    end
end
...
print("\n--- Initiating Direct Teleport to HauntletInterior ---")
...
-- This task uses WaitForChild to guarantee the HauntletInterior model is loaded.
task.spawn(handlePostTeleportMovement)

print("\nautomatic direct HauntletInterior teleport script initiated.")
